...
- The configuration expects the OWASP Core Rule Set to be installed under
  `/etc/nginx/modsec/crs`. Update the `Include` paths if the rules are stored in
...
- The configuration enables OCSP stapling and HSTS (one week). Increase the HSTS
  duration once confident in HTTPS-only operations.
